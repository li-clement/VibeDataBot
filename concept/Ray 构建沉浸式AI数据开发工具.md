...
* **核心特性**：  
  ...
  * **代码差异视图（Code Diff View）**：当 Agent 提议修改数据清洗逻辑时，TUI 展示代码的 Diff 视图，用户只需按 y 键确认，这直接借鉴了 Claude Code 的协作模式 2。  
  * **JSON 数据处理前后对比界面（Data Diff View）**：特别针对大语言模型微调常用的复杂 JSON/JSONL 数据，在应用每次清洗或脱敏规则后，原生界面以双栏（或内联视图）形式渲染样本的结构化对比。例如，直观高亮某一条 QA 数据在经过 Agent 处理后 `content` 字段的值变化，确保转换未破坏嵌套结构，赋予开发者极强的“所见即所得”核验感。
...
* **结构化 JSON 深度处理与验证闭环**：LLM 训练数据多为带有特定结构的 JSON（如对话历史树）。Agent 生成逻辑精确操作内部嵌套字典提取数据后，平台会主动抽取转换前后的微型样本集（Sample Batch）。将结果推送至 TUI 前端的 **JSON 处理前后对比组件**进行展示，利用富文本树状差异对比（Tree-Diff），使用户无需编写验证脚本即可洞察数据流中的结构变换全貌，确保数据管道的高可靠性。
...
* **高保真数据预检组件（JSON Diff Visualizer）**：Textual 原生支持树状节点和富文本高亮。针对 JSON 数据开发，我们在交互层专门打造了“数据处理前后对比界面”。在即将执行如“过滤 JSON 深层特定键”或“系统提示词统一微调”的批处理操作前，Agent UI 会拦截操作，展开两份漂亮排版的 JSON 面板，分别展示改动前与改动后的行级差异（如红色删除线代表移去，绿色背景代表新增结构），让“黑盒清洗”变为高可视化的协作审查。
...
* **复杂 JSON 的精准操作与对比确认流 (Confirmation Flow)**：现代 AI 数据处理涉及极多基于特定 JSON Schema（如 ShareGPT 格式）的规则重构。Agent 将生成自定义的 Python 函数进行递归式遍历转化，并且通过引入类似于 `Sample-and-Diff` 的工作流，提取执行后的小切片样本（如 `ds.take(2)`）。随后终端会挂起控制权（Yielding Control），系统界面自动进入“JSON前后状态对比模式”，渲染数据洗牌过程的 diff。工程师只需查阅该页面没有抛出结构异常并输入 `y`，即可继续下发成百上千个节点的完整集群 Task，大幅提升处理异构嵌套数据的作业信心。
* **合成数据 (Synthetic Data)**：  
